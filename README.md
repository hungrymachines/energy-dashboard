...
4. Your Home Assistant pulls the resulting schedule and, on every 30-minute boundary, applies it to each appliance. For HVAC that means setpoint, mode, and fan; for EV/battery/water heater it switches the device on or off. The plan sets a target for the room your sensor sits in, not for the thermostat's dial. Between those boundaries a check runs every five minutes: when the room reads more than half a degree off its target, the setpoint moves one degree toward closing the gap, and it repeats until the room lands where the plan asked. Your comfort range bounds every one of those commands, and the comfort check stays the safety net at the edges. The panel and cards in this package show what's running, what's coming next, and how much you save.
...
- **Pricing source** — one section with two modes:
  ...
  - **Export rates** — what your utility pays for power you send back to the grid, as a 48-slot half-hourly table in ¢/kWh. It sits under both modes, because export credit is priced separately from what you buy. Leave it empty if you are not paid for exports. Fill it in and the optimizer values your solar surplus against it, so an appliance only soaks up daylight generation when running then beats selling that power.
...
- **Solar** — no constraints to set, so the card carries **Edit appliance** instead: system size, tilt, azimuth, and the optional production sensor. The card shows the day's forecast generation curve, where that forecast came from (nameplate estimate, a forecast for your tilt and azimuth, or learned from your production sensor), the expected kWh, and — when you have entered export rates — the surplus and what it earns.
...
- **v3.8.1** — fix: **the comfort guard steps instead of jumping, and a slow API no longer costs you readings.** When the room climbs past the top of your comfort range, the guard now lowers the setpoint by 2 °F instead of dropping it straight to the bottom of the range. After the old jump, a window unit whose own sensor reads cooler than the room kept running well past the bottom edge, and the room drifted back out of range an hour later. If the room is still too warm ten minutes after a step, the guard lowers the setpoint another 2 °F, never below your range; heating mirrors this. Each request to the API now gives up after 20 seconds, and reading capture falls back to the appliance list it last fetched (refreshed every 30 minutes), so a slow or unreachable API delays the next push instead of skipping the reading. A timeout logs a warning that names the error rather than an unhandled error in the Home Assistant log. The calibration card no longer shows "NaN °F/hr" after a setpoint test; it says how many holds the test observed. No backend change required.
- **v3.8.0** — feat: **the setpoint now follows the room.** Your plan says what temperature the room should hold each half hour. Until now the integration handed that number to the thermostat as a setpoint, which is the same thing only when the unit's own sensor agrees with the room — on a window unit or mini-split it usually reads a few degrees off, so the room settled somewhere other than the plan. The integration now watches the room sensor you assigned to that appliance and moves the unit's setpoint a degree at a time until the room reaches the target, at most once every ten minutes, and never outside your comfort range. Within half a degree of the target it leaves the unit alone. Nothing changes if you have not assigned a sensor: the thermostat's own reading is used, as before. The comfort guard still outranks the loop — while the guard is holding an edge, tracking stands down — and the half-hour setpoint from the plan is still what gets applied at each boundary. Every reading push now also reports the target the plan asked for, so a room the unit cannot reach is visible in support instead of reading as a comfort setting nobody likes. Requires a backend that ships room targets (API migration 070).
- **v3.7.0** — feat: **the comfort guard now asks the unit to run, and the panel says where its numbers come from.** When the room leaves your comfort range, the guard used to command the edge it had just crossed — which changes nothing on a window unit or mini-split whose own thermostat reads a few degrees off the room, so the room stayed out of range. It now commands the far edge, so the unit actually runs; how far it runs is unchanged, because the guard still lets go as soon as the room is back inside the crossed edge by a degree. If you gave an appliance its own room-temperature sensor, that sensor is now the authoritative reading everywhere — what gets sent to us, and what the guard measures against — instead of the thermostat's internal reading. Savings now read "est. N% savings today" with the basis on hover, since the figure is modelled from your baseline at the day's prices rather than metered. Diagnostics gains an Integration and API build line for support conversations, and every reading push identifies the software that sent it. Requires a backend with the phase-1 control changes (API migrations 062-064).
- **v3.6.0** — feat: **same-day schedule changes now reach your thermostat within minutes, plus a Price spike guard toggle.** On a real-time rate like ComEd Hourly Pricing, the price you are billed is set as the hour unfolds, and it sometimes runs far above the forecast published the day before. When the backend spots one of those spikes developing and re-plans the rest of your day, your home now picks that plan up on its regular five-minute check instead of waiting for the next half-hour boundary. Turn it off any time in Settings → **Price spike guard**; your comfort range always applies either way, and the schedule still follows the day-ahead prices the rest of the time. The integration's own timing is simpler too: the setpoint is applied on the half hour and checked once a minute later — if the command did not stick, it is sent again — and the comfort-range check runs on the five-minute marks in between. Requires a backend with real-time price monitoring (API migration 061).
- **v3.5.0** — feat: **solar production sensor, export rates, and a solar generation card.** Point your solar appliance at a `sensor.*` reporting the array's instantaneous output in W or kW (picked when you add or edit the appliance) and the integration captures it every 5 minutes alongside every other appliance reading. The backend then learns your array's real yield and shading instead of estimating from the nameplate rating, and the solar card names which forecast it used — nameplate estimate, a forecast for your tilt and azimuth, or learned from your sensor. New **Export rates** table in Settings → Pricing: 48 half-hourly ¢/kWh values for what your utility pays on power you send back, entered separately from what you buy and available under both pricing sources; with it set, the optimizer values daylight surplus against the export price rather than treating it as free. The solar card plots the day's generation curve over the price bars and shows expected kWh, plus the surplus and its dollar value once export rates are entered. Requires a backend with solar production learning and export rates (API migrations 059-060). Also: **ComEd Delivery Time-of-Day pricing.** With Source = Dynamic, a new **Delivery plan** picker swaps the flat delivery estimate for ComEd's four daily delivery windows: pick your rate class (single- or multi-family, standard or electric heat) and the four period prices prefill from the published rates — each editable, so you can match them to the Distribution Facility Charge on your own bill. The delivery charge estimate then covers only the non-delivery extras (about 2¢/kWh is typical). Requires API migrations 057-058.
