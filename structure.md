...
|---|---|
| `hvac` | `{ intervals: number[48], high_temps: number[48], low_temps: number[48], setpoint_temps: number[48], room_target_temps: number[48] }` (Fahrenheit; optional `hvac_mode_schedule` / `fan_mode_schedule` string arrays when the user opts in) |
...
**HVAC carries two temperature arrays, and they are not the same number.** `setpoint_temps[i]` is what to command the thermostat. `room_target_temps[i]` (backend US-CTL-040, API migration 070) is the temperature the plan wants the *room* to reach. On a plant-family unit they differ by the backend's fitted per-unit sensor gap — the unit satisfies on its own sensor, which reads a few degrees off the room — and on a closed-loop OFF slot the room target is the far band edge the slot was priced as holding, not wherever the trajectory drifts. Both are clamped to the day's band, so the gap you can observe between them shrinks wherever the trajectory already rides an edge.

`scheduler.py` uses both. `apply_current_slot` commands `setpoint_temps[slot]` at each :00/:30 boundary, unchanged. Between boundaries `comfort_watchdog` runs `_track_room_target`, which reads the appliance's assigned room sensor (falling back to the thermostat's own reading) and steps the commanded setpoint by `ROOM_TRACK_STEP_F` toward the room target: outside a `ROOM_TRACK_DEADBAND_F` band, at most once per `comfort.MIN_ON_SECONDS` per entity, clamped to `[low_temps[slot], high_temps[slot]]`, and skipped entirely while the comfort guard's latch is active, while the slot's `hvac_mode_schedule` value is OFF, or when the day's `mode` is not cool/heat/auto. A plan carrying no `room_target_temps` (an API older than US-CTL-040) tracks nothing — the setpoint path alone still runs. The readings push reports the slot's room target back as `room_target` beside `target_temp`, so a room the unit cannot reach is visible in fleet diagnostics rather than reading as a comfort setting nobody likes.
